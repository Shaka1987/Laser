...
{
	if (!m_wndParameter.Create(_T("²ÎÊý"), this, CRect(0, 0, 150, 100), TRUE, ID_VIEW_PARAMETERWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI/*, AFX_CBRS_REGULAR_TABS,  AFX_CBRS_CLOSE| AFX_CBRS_AUTOHIDE*/))
	{
		return FALSE; // failed to create
	}
	return TRUE;
...
{
	if (!m_wndPLC.Create(_T("PLC ÌÝÐÎÍ¼"), this, CRect(0, 0, 150, 100), TRUE, ID_VIEW_PLCWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI))
	{
		return FALSE;
	}
	return TRUE;
