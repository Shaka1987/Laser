...
{
	if (!m_wndPLC.Create(_T("PLC 梯形图"), this, rect, TRUE, ID_VIEW_PLCWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
	{
		return FALSE;
	}
	return TRUE;
...
{
	if (!m_wndMonitor.Create(_T("监控面板"), this, TRUE, MAKEINTRESOURCE(IDD_MONITOR), WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI, IDD_MONITOR/*, AFX_CBRS_REGULAR_TABS, AFX_CBRS_CLOSE | AFX_CBRS_AUTOHIDE*/))
	{
		return FALSE;
	}
	return TRUE;
